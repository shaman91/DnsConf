...
#### Домены-цели для NextDNS

`REDIRECT` также принимает формат `домен-цель исходный-домен`:
...
Это расширение формата DnsConf, **не синтаксис Windows hosts**. NextDNS разрешает
домен-цель; импортёр не фиксирует его текущие IP. Родитель охватывает себя и все
поддомены. Родительская запись с доменом-целью подавляет последующие дочерние
записи, в том числе из следующих источников. Явные дочерние исключения размещайте
перед родителем. Для IP-родителей и первого точного совпадения поведение прежнее.
`EXCLUDE_REDIRECT` сохраняет приоритет. Некорректные доменные пары и циклы
останавливают обработку до очистки rewrite. Cloudflare отклоняет домены-цели до
изменения правил и списков. Формат `BLOCK` не расширяется до CNAME.

Код импортёра и доменные строки необходимо публиковать вместе: старый импортёр
пропускает эти строки и при `PRUNE_REDIRECT=true` может удалить старые rewrite.

Локальный dry-run полного набора источников без сети, ключей и применения:
...
Все аргументы — локальные файлы; JSON снимка содержит объекты с `name` и `content`,
файл исключений — по одному значению `EXCLUDE_REDIRECT` на строку. Результат JSON
содержит точные добавления, удаления, замены и ожидаемый полный профиль при prune.
Утилита ничего не применяет, не пишет файлы и не использует API.
...
Добавьте перед доменом `=`, чтобы исключить только точный домен, сохранив его поддомены, например `=tiktok.com`.
